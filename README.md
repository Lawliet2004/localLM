...
On Windows, application data normally resides in `%APPDATA%/app.locallm.desktop`: `locallm.sqlite`, installed skills, encrypted credential files and `runtime.log`. Windows Credential Manager protects the credential encryption key. Conversations and tool audits are not encrypted by the application. Unsent drafts use local WebView storage. When the loaded runtime supports it, `kv-slots/` holds each conversation's saved KV cache (processed prompt state, so it contains that conversation's content). These files are also unencrypted, are deleted with their conversation or from **Models & runtime → Runtime → Saved conversation cache**, and are capped by a disk budget. `checkpoints/` holds shadow git repositories with copies of workspace files from before and after each agent turn that could modify the workspace. This is what powers **Review → Changes by turn** and revert. They are separate from your own git repository, respect `.gitignore`, skip files over 5 MB, keep the latest 100 turns per workspace, and can be turned off or deleted from that panel.
