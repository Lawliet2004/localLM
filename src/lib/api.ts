...
export const api = {
  ...
  kvCacheUsage: () => invoke<KvCacheUsage>('kv_cache_usage'),
  saveKvCacheSettings: (settings: KvCacheSettings) => invoke<KvCacheUsage>('save_kv_cache_settings', { settings }),
  clearKvCache: () => invoke<KvCacheUsage>('clear_kv_cache'),
  listCheckpoints: (conversationId: string) => invoke<Checkpoint[]>('list_checkpoints', { conversationId }),
  checkpointDiff: (id: string) => invoke<CheckpointDiff>('checkpoint_diff', { id }),
  revertCheckpoint: (id: string) => invoke<RevertResult>('revert_checkpoint', { id }),
  checkpointSettings: () => invoke<{ enabled: boolean }>('checkpoint_settings'),
  saveCheckpointSettings: (settings: { enabled: boolean }) => invoke<{ enabled: boolean }>('save_checkpoint_settings', { settings }),
  clearCheckpoints: () => invoke<void>('clear_checkpoints'),
