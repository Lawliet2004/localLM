...
export function ToolsSettings({ scopeLabel, onBack, selected, onChange, selectedTools = [], onToolsChange, accessMode = 'ask', onAccessModeChange, preset = 'standard', onPresetChange, busy, loading = false, saving = false, onNavigate }: ToolsSettingsProps) {
  ...
  return <div className="settings-page tools-settings">
    ...
    <div className="tool-group"><h2>Code execution</h2><p>Run code locally using your workspace, or use a Daytona cloud sandbox.</p>
      ...
      {!pending && !hasDaytona && <p className="tool-unavailable" role="status">Save a Daytona key to enable cloud execution. {onNavigate && <button type="button" className="secondary" onClick={() => onNavigate('execution')}>Open Execution</button>}</p>}
    </div>
    ...
    <section className="tool-group"><h2>Permissions</h2><p>Choose when the assistant should ask before using a tool.</p><PermissionSelector accessMode={accessMode} onAccessModeChange={onAccessModeChange} busy={busy} nativeAvailable={nativeAvailable} /></section>
