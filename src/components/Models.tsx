...
export function Models({ config, preferences, runtime, busy, onSaveConfig, onSavePreferences, onLoad, onUnload }: Props) {
  ...
  const [tab, setTab] = useState<'model' | 'runtime' | 'generation'>('model');
  return <div className="settings-page">
    ...
    {tab === 'runtime' ? <RuntimeForm initial={config} busy={busy} onSave={async value => { try { setError(''); await onSaveConfig(value); setNotice('Runtime configuration saved. Reload the model to apply it.'); } catch (e) { setError(String(e)); } }} /> :
