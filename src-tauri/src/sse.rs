#[derive(Default)]
pub struct SseDecoder { pending: Vec<u8>, data: Vec<String>, event_size: usize }
impl SseDecoder {
    pub fn push(&mut self, bytes: &[u8]) -> Result<Vec<String>, String> {
        let mut events = Vec::new();
        for byte in bytes {
            self.event_size += 1;
            if self.event_size > 1_048_576 { return Err("Stream event exceeds 1 MiB.".into()); }
            if *byte != b'\n' { self.pending.push(*byte); continue; }
            if self.pending.last() == Some(&b'\r') { self.pending.pop(); }
            let line = std::str::from_utf8(&self.pending).map_err(|_| "Invalid UTF-8 in event stream.")?;
            if line.is_empty() {
    ...
    pub fn finish(&self) -> Result<(), String> {
        if !self.pending.is_empty() || !self.data.is_empty() { return Err("Model disconnected in the middle of a stream event.".into()); }
        Ok(())
    }
}
...
mod tests {
    ...
    fn preserves_unicode_across_network_boundaries_and_accepts_crlf() {
        ...
        let mut events = Vec::new();
        for byte in bytes { events.extend(decoder.push(&[*byte]).unwrap()); }
