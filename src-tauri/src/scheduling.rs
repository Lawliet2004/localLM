...
pub fn parse_cron(cron: &str) -> Result<Cron, String> {
    ...
    Ok(Cron {
        ...
        dom_star: parts[2].trim() == "*",
        dow_star: parts[4].trim() == "*",
    })
}

/// Next minute-aligned UTC timestamp strictly after `from_secs` matching the
/// schedule, searched up to one year ahead (covers weekly + monthly patterns).
pub fn cron_next(cron: &str, from_secs: i64) -> Result<i64, String> {
    ...
    for _ in 0..525_600 {
        ...
        // POSIX cron day semantics: when both DOM and DOW are restricted the
        // match is a union; when one side is `*` the other side alone decides.
        let day_matches = match (parsed.dom_star, parsed.dow_star) {
            (true, true) => true,
            (true, false) => parsed.weekdays.contains(&weekday),
            (false, true) => parsed.days.contains(&day),
            (false, false) => parsed.days.contains(&day) || parsed.weekdays.contains(&weekday),
        };
    ...
    Err("No matching cron time in the next year.".into())
}
...
pub fn is_due(schedule: &Schedule, now_secs: i64) -> bool {
    ...
    match schedule.last_run_at {
        None => schedule.run_once || cron_next(&schedule.cron, schedule.created_at / 1000)
            .map(|next| next <= now_secs).unwrap_or(false),
...
fn parse_http(head: &str, body: &[u8]) -> Result<HttpRequest, String> {
    ...
    let _ = parts.next() != Some("HTTP/1.1") && parts.next().is_some();
...
mod tests {
    ...
    #[test]
    fn weekly_and_monthly_schedules_survive_past_66_hours() {
        // 2026-01-01 is a Thursday; derive day timestamps from its midnight.
        let jan1 = 1_767_225_600i64;
        // Tuesday after a Monday fire: next Monday midnight is +6 days.
        let tuesday = jan1 + 5 * 86_400; // 2026-01-06 (Tuesday) 00:00 UTC
        let next = cron_next("0 0 * * 1", tuesday).unwrap();
        let fields = utc_fields(next);
        assert_eq!(fields.4, 1); // Monday
        assert!(next - tuesday > 66 * 3600, "must reach past the old 66h horizon");
        assert!(next - tuesday <= 7 * 86_400);
        // A weekday-only schedule must not fire on the wrong day.
        let saturday = jan1 + 9 * 86_400; // 2026-01-10 (Saturday) 00:00 UTC
        let next = cron_next("0 0 * * 1", saturday).unwrap();
        let fields = utc_fields(next);
        assert_eq!(fields.4, 1); // Monday
        assert_eq!(next - saturday, 2 * 86_400);
        // Monthly `0 0 1 * *` from mid-month lands on the 1st within 31 days.
        let mid_jan = jan1 + 2 * 86_400; // 2026-01-03 00:00 UTC
        let next = cron_next("0 0 1 * *", mid_jan).unwrap();
        let fields = utc_fields(next);
        assert_eq!(fields.2, 1);
        assert!(next - mid_jan <= 31 * 86_400);
    }
    #[test]
