...
pub fn registry() -> Vec<(String, String, Value)> {
    vec![
        ("todo_write".into(), "Replace the conversation todo list (logged state). Pass the FULL list each time.".into(),
        ...
        ("goal_set".into(), "Set the long-running objective for this conversation (logged, injected every turn).".into(),
        ...
        ("ask_user".into(), "Ask the user a question (Allow = first option, Deny = declined).".into(),
...
pub fn is_trusted_read(alias: &str) -> bool {
    matches!(alias, "list_agents" | "list_subagent_models" | "memory_recall" | "file_search"
        | "preset_guide" | "schedule_list" | "terminal_send")
}
...
pub async fn execute(ctx: &HarnessCtx<'_>, alias: &str, args: Value) -> Result<HarnessOutcome, String> {
    ...
    // Short DB locks only: no MutexGuard may cross an await (Send).
    match alias {
        "todo_write" => {
            let todos = crate::plans::parse_todo_write(&args)?;
            ctx.state.database()?.save_todos(&ctx.conversation_id, &todos)?;
        ...
        "goal_set" => {
            ...
            ctx.state.database()?.save_goal(&ctx.conversation_id, &objective)?;
            Ok(HarnessOutcome::value(json!({"goal": objective}))
                .event("plan", json!({"kind": "goal_set"})))
        }
        "goal_clear" => {
            ctx.state.database()?.clear_goal(&ctx.conversation_id)?;
        ...
        "send_message" => {
            ...
            let rows = ctx.state.database()?.subagent_runs_for_conversation(&ctx.conversation_id)?;
        ...
        "list_subagent_models" => {
            let allowlist = {
                ...
                crate::subagents::list_models(&store)?
            };
        ...
        "workflow_run" => {
            ...
            let parent = crate::subagents::ParentCtx {
            ...
            let value = crate::subagents::run_workflow(
                ctx.state, &ctx.snapshot, &parent, mode, steps, max_rounds.clamp(1, 8),
            )
            .await?;
        ...
        "ralph_run" => {
            ...
            let value = crate::subagents::run_ralph(
                ctx.state, &ctx.snapshot, &parent, &objective, max_rounds,
            )
            .await?;
        ...
        "web_fetch" => {
            ...
            Ok(HarnessOutcome::value(crate::sandbox::web_fetch(&url).await?))
        }
        "file_search" => {
        ...
        "memory_teach" => {
            ...
            let record = ctx.state.database()?.teach_fact(&scope, &fact, "model")?;
        ...
        "memory_recall" => {
            ...
            let facts = ctx.state.database()?.recall_facts(&scope, limit.min(50))?;
        ...
        "schedule_create" => {
            ...
            ctx.state.database()?.save_schedule(&schedule)?;
        ...
        "schedule_list" => {
            let schedules = ctx.state.database()?.schedules()?;
            Ok(HarnessOutcome::value(json!({"schedules": schedules})))
        }
        "schedule_run" => {
            let id = arg_str(&args, "id", 1, 64, "schedule_run")?;
            let schedule = ctx.state.database()?.schedules()?.into_iter().find(|item| item.id == id).ok_or("Unknown schedule.")?;
        ...
        "ask_user" => {
            ...
            (ctx.emit)(
                ...
                Some(json!({"id": approval_id, "connector": "Interaction", "name": "ask_user",
            ...
            if answer {
                Ok(HarnessOutcome::value(json!({"answer": options.first().cloned().unwrap_or_else(|| "yes".into())})))
            } else {
                Ok(HarnessOutcome::value(json!({"answer": "declined"})))
            }
        }
        "ptc_run" => execute_ptc(ctx, args).await,
        "docker_exec" => {
            let command = arg_str(&args, "command", 1, 8192, "docker_exec")?;
            let image = match args.get("image").and_then(|value| value.as_str()) {
                Some(custom) => custom.to_string(),
                None => ctx.state.database().map(|store| crate::sandbox::docker_image(&store)).unwrap_or_default(),
            };
...
async fn execute_subagent(ctx: &HarnessCtx<'_>, args: Value) -> Result<HarnessOutcome, String> {
    ...
    let (child_run_id, answer) =
        crate::subagents::run_child_inline(ctx.state, &ctx.snapshot, request, ctx.inherit_tools).await?;
...
async fn execute_ptc(ctx: &HarnessCtx<'_>, args: Value) -> Result<HarnessOutcome, String> {
    ...
    for (index, step) in steps.iter().enumerate() {
        ...
        let row = ctx.state.database()?.append_message(&ctx.conversation_id, "tool", &audit.to_string(), if allow { "streaming" } else { "complete" })?;
        if !allow {
            let denied = error_result("The user denied this program step. Remaining steps were skipped.".into());
            ctx.state.database()?.update_message(&row.id, &json!({"request": audit, "result": denied}).to_string(), "", "complete")?;
        ...
        ctx.state.database()?.update_message(&row.id, &json!({"request": audit, "result": bounded}).to_string(), "", "complete")?;
